...
const LINES = [
  ...
  "SERVICE: google_oauth.... UNAVAILABLE",
  "SERVICE: github_oauth.... UNAVAILABLE",
  "SERVICE: x_oauth......... NOT_IMPLEMENTED",
  "$ reason: OAuth integration still in progress",
  "$ estimated_completion: soon™",
  "$ meanwhile: use email/password to sign in",
]
...
export default function MaintenancePage() {
  ...
  useEffect(() => {
    ...
    if (currentChar < line.length) {
      const speed = line.startsWith("SERVICE") ? 25 : 20
    ...
    <div className="w-full h-screen bg-[#0a0a0a] flex flex-col items-center justify-center overflow-hidden relative font-mono">
      ...
      {glitchActive && (
        <div className="absolute inset-0 pointer-events-none z-20">
          <div className="absolute inset-0 bg-[#ff2d6f]/5 mix-blend-screen" />
          <div
            className="absolute w-full h-[3px] bg-[#c8ff00]/30"
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0">
        <h1
          className="text-[18vw] font-black text-white/[0.03] leading-none select-none transition-all duration-75"
          style={{
            textShadow: glitchActive
              ? "3px 0 #ff2d6f, -3px 0 #00d4ff"
              : "none",
      ...
      <div className="relative z-10 w-[90%] max-w-2xl">
        ...
        <div className="flex items-center gap-2 px-4 py-3 bg-[#1a1a1a] border-b border-white/10">
          ...
          <span className="ml-3 text-[10px] text-white/30 uppercase tracking-widest">
            ml-tutor — service-status
          </span>
        </div>

        {/* Body */}
        <div className="bg-[#111111] border border-white/10 border-t-0 p-6 min-h-[260px] max-h-[350px] overflow-y-auto">
          {displayedLines.map((line, i) => {
            const isUnavailable = line.includes("UNAVAILABLE") || line.includes("NOT_IMPLEMENTED")
            const isOk = line.includes("OK")

            return (
              <div key={i} className="mb-1 flex items-start gap-2">
                <span className="text-[#ff2d6f]/50 select-none shrink-0">
                  ...
                  {i === currentLine && currentLine < LINES.length && (
                    <span
                      className={`inline-block w-2 h-4 ml-0.5 align-middle ${
                        cursorVisible ? "bg-[#ff2d6f]" : "bg-transparent"
                      }`}
                    />
                  )}
                </span>
          ...
          {currentLine >= LINES.length && (
            <div className="mt-4 flex items-center gap-2">
              <span className="text-[#ff2d6f]/50">❯</span>
              <span
      ...
      <div className="absolute bottom-6 left-0 right-0 text-center z-10">
        <p className="text-[9px] font-mono uppercase tracking-[0.3em] text-white/15">
          ML Tutor — Social Login — Under Maintenance
        </p>
      </div>
    </div>
  )
